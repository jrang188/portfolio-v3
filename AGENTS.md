...
- **Git hooks:** `simple-git-hooks` sets a pre-commit hook that runs `npx lint-staged && nix fmt`. The `nix fmt` part will fail in environments without Nix installed — this does not affect development, but be aware if committing from this environment.
